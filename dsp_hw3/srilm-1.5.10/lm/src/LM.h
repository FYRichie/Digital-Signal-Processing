...
class LM : public Debug {
   ...
   public:
    ...
    virtual NgramCount pplCountsFile(File &file, unsigned order,
                                     TextStats &stats,
                                     const char *escapeString = 0,
                                     Boolean entropy = false) { return pplCountsFile(file, order, stats, escapeString, entropy,
    ...
    virtual void *contextID(const VocabIndex *context) {
        unsigned length;
        return contextID(context, length);
    };
    virtual void *contextID(const VocabIndex *context, unsigned &length) { return contextID(Vocab_None, context, length); };
    /* context used by LM */
    ...
    virtual _LM_FollowIter *followIter(const VocabIndex *context) { return new _LM_FollowIter(*this, context); };
...
class TruncatedContext {
   public:
    ...
    ~TruncatedContext() { ((VocabIndex *)myContext)[contextLength] = saved; };
