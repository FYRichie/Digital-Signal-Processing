...
Ngram::Ngram(Vocab &vocab, unsigned neworder)
    : LM(vocab), order(neworder), _skipOOVs(false), _trustTotals(false), contexts(vocab.numWords()) {
    if (order < 1) {
        order = 1;
    }
}
 ...
 */
LogP Ngram::contextBOW(const VocabIndex *context, unsigned length) {
    BOtrie *trieNode = &contexts;
    LogP bow = LogP_One;
...
Ngram::writeBinaryNode(BOtrie &node, unsigned level, File &file,
    ...
    unsigned offsetBytes = subtrieDepth == 0 ? 2 : subtrieDepth <= 3 ? 4
                                                                     : 8;
 ...
 */
void Ngram::mixProbs(Ngram &lm2, double lambda) {
    makeArray(VocabIndex, context, order + 1);
...
void Ngram::mixProbs(Ngram &lm1, Ngram &lm2, double lambda) {
    makeArray(VocabIndex, context, order + 1);
 ...
 */
void Ngram::pruneProbs(double threshold, unsigned minorder) {
    /*
 ...
 */
void Ngram::pruneLowProbs(unsigned minorder) {
    makeArray(VocabIndex, context, order);
