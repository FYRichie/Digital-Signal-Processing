 ...
 */
void disambiguateFile(File &file, VocabMap &map, LM &lm, VocabMap *counts) {
    char *line;
 ...
 * disambiguate it, and print out the result
 */
void disambiguateFileContinuous(File &file, VocabMap &map, LM &lm,
                                VocabMap *counts) {
    char *line;
 ...
 * disambiguate it, and print out the result
 */
void disambiguateTextMap(File &file, Vocab &vocab, LM &lm, VocabMap *counts) {
    char *line;
...
LM *makeMixLM(const char *filename, Vocab &vocab,
    ...
    if (useServer && strchr(filename, '@') && !strchr(filename, '/')) {
        ...
        lm = factored ? new ProductNgram((ProductVocab &)vocab, order) : new Ngram(vocab, order);
        assert(lm != 0);
